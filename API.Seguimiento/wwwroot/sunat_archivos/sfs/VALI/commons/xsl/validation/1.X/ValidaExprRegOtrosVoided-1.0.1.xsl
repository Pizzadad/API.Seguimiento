...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="sac:VoidedDocumentsLine">
    ...
    <xsl:choose>
      <!-- /VoidedDocuments/sac:VoidedDocumentsLine/sac:DocumentSerialID Si "Tipo de documento" es 01, el valor del Tag UBL empieza con un valor diferente a "R"
            ERROR 2674 -->
      <xsl:when test="cbc:DocumentTypeCode='20'">
        <xsl:call-template name="existAndRegexpValidateElement">
          <xsl:with-param name="errorCodeNotExist"
                          select="'2311'"/>
          <xsl:with-param name="errorCodeValidate"
      ...
      <!-- /VoidedDocuments/sac:VoidedDocumentsLine/sac:DocumentSerialID Si "Tipo de documento" es 40, el valor del Tag UBL empieza con un valor diferente a "P"
            ERROR 2675 -->
      <xsl:when test="cbc:DocumentTypeCode='40'">
        <xsl:call-template name="existAndRegexpValidateElement">
          <xsl:with-param name="errorCodeNotExist"
                          select="'2311'"/>
          <xsl:with-param name="errorCodeValidate"
          ...
          <xsl:with-param name="node"
                          select="sac:DocumentSerialID"/>
          <xsl:with-param name="regexp"
                          select="'(^[P][A-Z0-9]{3}$)|(^[\d]{1,4}$)'"/>
          <xsl:with-param name="descripcion"
                          select="concat('Error en la linea:', $nroLinea, '. ')"/>
        </xsl:call-template>
      </xsl:when>

      <!-- /VoidedDocuments/sac:VoidedDocumentsLine/sac:DocumentSerialID Si "Tipo de documento" es 04, el valor del Tag UBL empieza con un valor diferente a "P"
            ERROR 2345 -->
      <xsl:when test="cbc:DocumentTypeCode='04'">
        <xsl:call-template name="existAndRegexpValidateElement">
          <xsl:with-param name="errorCodeNotExist"
                          select="'2311'"/>
          <xsl:with-param name="errorCodeValidate"
    ...
    <xsl:call-template name="isTrueExpresion">
      ...
      <xsl:with-param name="node"
                      select="cbc:LineID"/>
      <xsl:with-param name="expresion"
                      select="count(key('by-Billing-in-line', concat(cbc:DocumentTypeCode,sac:DocumentSerialID, number(sac:DocumentNumberID)))) &gt; 1"/>
      <xsl:with-param name="descripcion"
                      select="concat('Error en la linea:', position(), '. ')"/>
    </xsl:call-template>
