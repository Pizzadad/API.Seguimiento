...
xmlns:dp="http://www.datapower.com/extensions" extension-element-prefixes="dp" exclude-result-prefixes="dp dyn regexp date func" version="1.0">
  ...
  <xsl:template name="existElement">
    ...
    <xsl:if test="not(string($node))">
           <xsl:call-template name="addResultWithNode">
               <xsl:with-param name="isError" select="$isError" />
               <xsl:with-param name="code" select="$errorCodeNotExist" />
               <xsl:with-param name="message" select="$descripcion" />
               <xsl:with-param name="node" select="$node"/>
           </xsl:call-template>
    </xsl:if>
  </xsl:template>
  ...
  <xsl:template name="existAndRegexpValidateElement">
    ...
    <xsl:otherwise>
    <xsl:if test="not(app:Matches($regexp, string($node)))">
      <xsl:choose>
  ...
  <xsl:template name="regexpValidateElementIfExist">
    ...
    <xsl:param name="isError" select="true()"/>
    <xsl:param name="descripcion" select="''"/>
    <xsl:if test="count($node) &gt;= 1 and not(app:Matches($regexp, string($node)))">
      <xsl:call-template name="addResultWithNode">
        <xsl:with-param name="isError" select="$isError" />
        <xsl:with-param name="code" select="$errorCodeValidate" />
        <xsl:with-param name="message" select="$descripcion" />
        <xsl:with-param name="node" select="$node"/>
      </xsl:call-template>
    </xsl:if>
  ...
  <xsl:template name="findElementInCatalog">
    ...
    <xsl:if test="count($idCatalogo) &gt;= 1 and count(document($url_catalogo)/l/c[@id=$idCatalogo]) &lt; 1 ">
            <xsl:call-template name="addResultWithNode">
                <xsl:with-param name="isError" select="$isError" />
                <xsl:with-param name="code" select="$errorCodeValidate" />
                <xsl:with-param name="message" select="concat($descripcion,': ', $errorCodeValidate)" />
                <xsl:with-param name="node" select="$idCatalogo"/>
            </xsl:call-template>
    </xsl:if>
  </xsl:template>
  <!-- Template que sirve para validar la existencia del valor de un tag dentro de un catalogo -->
  <xsl:template name="findElementInCatalogProperty">
    ...
    <xsl:if test="not(app:ExistsItemCatalogo(concat('cat_',$catalogo,'.xml'), concat('/l/c[', $vCondition, ']')))">
            <xsl:call-template name="addResultWithNode">
                <xsl:with-param name="isError" select="true()" />
                <xsl:with-param name="code" select="$errorCodeValidate" />
                <xsl:with-param name="message" select="concat($descripcion,': condicion:',$dynEval,' Valor no se encuentra en el catalogo: ',$catalogo,', ID: ', $idCatalogo, '  (nodo: &quot;',name($idCatalogo/parent::*),'/', name($idCatalogo), '&quot; propiedad ',$propiedad,': &quot;', $valorPropiedad, '&quot;)')" />
                <xsl:with-param name="node" select="$idCatalogo" />
            </xsl:call-template>
    </xsl:if>
  </xsl:template>
  ...
  <xsl:template name="verifyAfiliacion">
    ...
    <xsl:if test="string($resp/result/responsecode) != '200'">
            <xsl:call-template name="addResult">
                ...
                <xsl:with-param name="message" select="concat('El servicio: ',$urlService, ' no esta disponible')" />
            </xsl:call-template>
    </xsl:if>
  </xsl:template>
  <xsl:template name="isDateBefore">
    ...
    <xsl:if test="(date:seconds(date:difference(concat($endDateNode,'-00:00'),$startDateNode)) &lt; 0)">
            <xsl:call-template name="addResult">
                <xsl:with-param name="isError" select="$isError" />
                <xsl:with-param name="code" select="$errorCodeValidate" />
                <xsl:with-param name="message" select="concat($descripcion,': errorCode ', $errorCodeValidate,' (nodo: &quot;',name($startDateNode/parent::*),'/', name($startDateNode), '&quot; valor: &quot;', $startDateNode, '&quot;)')" />
            </xsl:call-template>
    </xsl:if>
  </xsl:template>
  ...
  <xsl:template name="isDateBeforeIfExist">
    ...
    <xsl:if test="count($startDateNode) &gt;= 1 and (date:seconds(date:difference(concat($endDateNode,'-00:00'),$startDateNode)) &lt; 0)">
            <xsl:call-template name="addResult">
                <xsl:with-param name="isError" select="$isError" />
                <xsl:with-param name="code" select="$errorCodeValidate" />
                <xsl:with-param name="message" select="concat($descripcion,': ', $errorCodeValidate,' (nodo: &quot;',name($startDateNode/parent::*),'/', name($startDateNode), '&quot; valor: &quot;', $startDateNode, '&quot;)')" />
            </xsl:call-template>
    </xsl:if>
  </xsl:template>
  ...
  <xsl:template name="validateCertContribuyente">
    ...
    <xsl:if test="$errorCode != 0">
      <xsl:call-template name="addResult">
          ...
          <xsl:with-param name="message" select="concat('Validation Cert Serial error: ', $errorCode ,' rucBillcertserial: ',$rucBillCertSerialExaDecimal ,' rucPseBillcertserial: ',$rucPseCertSerialExaDecimal,' issueDate: ', $issueDate)" />
      </xsl:call-template>
    </xsl:if>
  </xsl:template>
  <xsl:template name="findElementInCatalogPropertyxComponente">
    ...
    <xsl:if test="count(dyn:evaluate($dynEval)) &lt; 1 ">
            <xsl:call-template name="addResult">
                <xsl:with-param name="isError" select="$isError" />
                <xsl:with-param name="code" select="$errorCodeValidate" />
                <xsl:with-param name="message" select="concat($descripcion,': condicion:',$dynEval,' Valor no se encuentra en el catalogo: ',$catalogo,', ', $propiedadCatalogo , ': ', $idCatalogo, '  (nodo: &quot;',name($idCatalogo/parent::*),'/', name($idCatalogo), '&quot; propiedad ',$propiedad,': &quot;', $valorPropiedad, '&quot;)')" />
            </xsl:call-template>
    </xsl:if>
  </xsl:template>
</xsl:stylesheet>
