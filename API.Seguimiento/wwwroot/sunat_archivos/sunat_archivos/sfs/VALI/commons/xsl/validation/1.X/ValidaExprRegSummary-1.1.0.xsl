...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="sac:SummaryDocumentsLine">
    ...
    <!-- Total valor de venta - operaciones exportacion -->
    ...
    <xsl:if test="$totVentaOservicioPrestado">
      <xsl:variable name="sumaCargosTributos"
                    select="number($totOtrosTributos) + number($totIGV) + number($totISC) + number($totOtrosCargos)  + number($totICBPER) + number($totVentaOperInafectas) + number($totVentaOperExoneradas) + number($totVentaOperExportacion) + number($totVentaOperGravadas)"/>
      <xsl:call-template name="isTrueExpresion">
        <xsl:with-param name="errorCodeValidate"
                        select="'4027'"/>
        <xsl:with-param name="node"
